# Screenshots

Add product screenshots here before publishing the GitHub release.

Suggested filenames (referenced by the root README):

## Mobile / general

| File | Description |
|------|-------------|
| `login.png` | Login screen (light) |
| `login-dark.png` | Login screen (dark) |
| `dashboard.png` | Executive dashboard (light) |
| `dashboard-dark.png` | Executive dashboard (dark) |
| `notifications.png` | Notification center |
| `reports-center.png` | Reports Center hub |
| `overtime.png` | Overtime tracking |
| `voice-notes.png` | Stage-based voice notes |
| `work-orders.png` | Work orders list / detail |
| `inventory.png` | Inventory / spare parts |
| `assets.png` | Assets registry |
| `pm.png` | Preventive maintenance |
| `reports.png` | Service reports |
| `roles.png` | Roles & Permissions |
| `settings.png` | Settings hub |

## Enterprise features

| File | Description |
|------|-------------|
| `voice-settings.png` | Overtime / voice configuration settings |
| `config-lab.png` | Configuration Testing Lab |
| `excel-export.png` | Enterprise Excel reports |

## Desktop

| File | Description |
|------|-------------|
| `desktop-dashboard.png` | Desktop Shell with dashboard |
| `desktop-overtime.png` | Overtime detail (timeline / map) |
| `desktop-work-order.png` | Work order detail split layout |
| `desktop-inventory.png` | Inventory hub (stats + action cards) |
| `desktop-users.png` | User Management hub |
| `desktop-roles.png` | Roles & Permissions hub |
| `desktop-settings.png` | Settings split panel |

Recommended size: 1080×2400 (phone) or 1600×1000 (tablet / desktop landscape).
