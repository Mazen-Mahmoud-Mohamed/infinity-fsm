...
class FlutterLocalNotificationsWindows extends WindowsNotificationsBase {
  ...
  /// Keeps the FFI launch callback alive for the process lifetime.
  ///
  /// Without a strong reference, the [NativeCallable] can be GC'd while COM
  /// still holds the native function pointer.
  NativeCallable<NativeNotificationCallbackFunction>? _launchCallable;
  ...
  Future<void> show({
    ...
    // Native already logged WinRT failures; do not throw across Flutter.
    if (!result) {
      return;
    }
  });
