...
struct RegistryHandle {
  ...
  // Never abort on RegCloseKey failure — WINRT_VERIFY_ can terminate the process.
...
bool RegisterCallback(const std::string& guid, NativeNotificationCallback callback) {
  ...
  const HRESULT hr = CoRegisterClassObject(
    rclsid, factory, CLSCTX_LOCAL_SERVER, REGCLS_MULTIPLEUSE, &registration
  );
  if (FAILED(hr)) {
    char buffer[160];
    std::snprintf(
      buffer, sizeof(buffer),
      "[flutter_local_notifications_windows] CoRegisterClassObject failed hr=0x%08X\n",
